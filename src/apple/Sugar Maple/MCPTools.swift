...
enum MCPTools {
    ...
    static func list(transactionSchema: Any, commentsQuerySchema: Any) -> [[String:Any]] {
        ...
        var transaction = tool("transaction.apply","Apply an atomic, undoable batch of page/folder/node/token/comment edits. Read document.get first. IDs returned correspond to added pages/folders/nodes/comment threads. Retry with the identical requestId and payload.")
